...
export function registerHaFacade(registry) {
  ...
  registry.register(
    'ha_execute_service',
    'Execute a Home Assistant service call to control any device — turn lights on/off, ' +
    'adjust brightness or colour, set thermostat temperature, lock/unlock doors, run ' +
    'scripts or scenes, trigger automations, etc. ' +
    ...
    '{domain:"climate",service:"set_temperature",service_data:{entity_id:"climate.office",temperature:72}}',
    {
      ...
      properties: {
        domain: {
          type: 'string',
          description: 'HA domain, e.g. "light", "switch", "climate", "lock", "scene", "script", "automation".',
        },
        service: {
          type: 'string',
    ...
    async ({ domain, service, service_data }) => {
      ...
      try {
        const result = await registry.execute(callTool, { domain, service, service_data })
